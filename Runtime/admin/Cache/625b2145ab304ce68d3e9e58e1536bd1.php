...
<div class="container">
    <div class="page-header"><h4>微信秒杀管理</h4></div>
    <div class="row">
        <div class="span12">
            <div class="pull-left">
                <a href="<?php echo U($module_name.'/add');?>" class="btn btn-primary">新增活动</a>
            </div>
        </div>
        <div class="span12" style="margin-top:10px;">
            ...
            <table class="table table-hover">
                ...
                <tfoot>
                    <tr>
                        <td colspan="14"><div class="row pagination page"><div class="span12"><?php echo ($page); ?></div></div></td>
                    </tr>
                </tfoot>
                <tbody>
                <?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr id='tr_<?php echo ($vo["id"]); ?>'>
                        ...
                        <td><?php echo ($vo["goods_num"]); ?></php></td>
                        <td><?php echo get_seckill_num($vo['id']);?></td>
                        <td><?php echo get_seckill_surplus($vo['id']);?></php></td>
                        <td><?php if(empty($vo['begin_time'])) echo $vo['begin_time']; else echo get_date_full($vo['begin_time']); ?></td>
                        <td><?php if(empty($vo['stop_time'])) echo $vo['stop_time']; else echo get_date_full($vo['stop_time']); ?></td>
                        <td><?php echo get_seckill_status($vo['id']);?></td>
                        <td><?php if($vo['wx_user_num']==0) echo '不限制'; else echo $vo['wx_user_num']; ?></td>
                        <td><?php echo get_status_img($vo['status']);?></td>
                        <td><?php echo ($vo["sort"]); ?></td>
                        <td><?php echo get_seckill_pv($vo['id']);?></td>
                    <td>
                        <!--<a href='<?php echo U($module_name.'/view?id='.$vo['id']);?>'>查看</a>&nbsp;&nbsp;-->
